...
const categories = [
  ...
  },
  {
    key: "pickles",
    title: "Pickles",
    img: "https://images.unsplash.com/photo-1625944528087-74ccfb7a5f6f?q=80&w=1200&auto=format&fit=crop",
...
export default function SwagruhaGrid() {
  return (
    <section id="swagruha" className="bg-amber-50 py-14 sm:py-20">
      <div className="mx-auto max-w-7xl px-4 sm:px-6">
        <div className="flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-end">
          <div>
            <h2 className="text-2xl font-bold tracking-tight text-stone-900 sm:text-4xl">Swagruha Foods</h2>
          ...
          <a href="#contact" className="inline-flex items-center gap-2 rounded-lg bg-stone-900 px-4 py-2.5 text-sm font-semibold text-white shadow hover:bg-stone-800">
        ...
        <div className="mt-6 grid gap-4 sm:mt-8 sm:grid-cols-2 lg:grid-cols-4">
          {categories.map((cat) => (
            <a key={cat.key} href="#contact" className="group overflow-hidden rounded-2xl border border-amber-200 bg-white shadow-sm transition hover:shadow-md">
              ...
              <div className="p-4">
                <h3 className="text-base font-semibold text-stone-900 sm:text-lg">{cat.title}</h3>
                <p className="mt-1 text-xs text-stone-600 sm:text-sm">{cat.desc}</p>
              </div>
            </a>
          ))}
        </div>
